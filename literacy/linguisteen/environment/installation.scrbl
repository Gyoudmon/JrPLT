...
本小节列出的软件所有学生都要安装。
有了这些软件，学生只需要花费1-3次课的学习成本就能显著减少后续学习过程中的痛苦程度。
...
基本都是 Racket 先有，然后才被其他语言的作者抄到自己的语言里去的。

Racket 是我最常用的语言，它是现代 Lisp 最前卫的方言之一，
也是“程序语言理论”领域最专业的语言之一。
实际上，Racket 正是在探索青少年编程教育的过程中才慢慢从 Scheme 独立出来，
成为独树一帜的 Lisp 方言，
既保留了对数学理论纯粹性的追求，
又不失工程实践的实用性。

为降低初学者的入门痛苦程度，
我用 Racket 写了很多趁手的工具来帮助学生学习。
Racket 在本系列课程中的作用有：

@itemlist[
 #:style 'compact

 @item{充当 C++ 构建、测试工具，一行简短的命令搞定所有恼人的编译细节。}
 
 @item{充当专业流程图绘制和生成工具。此工具亦可替代 Python Turtles 库。}
 
 @item{用于打开学生的学术眼界。比如以更直接和切题的方式回答“数学和编程的内在联系”。}
]
...
@itemlist[
 ...
 @item{编写 C++/Python 或其他语言时，字体、配色、操作风格一致。
  ...
  我的课件、学生手册显然也不应该为每一种开发软件看到的效果都截个图。}
...
@centered{@emph{只学 Python 的学生可以跳过。}}

Code::Blocks 和 Dev-C++ 都是信奥赛考纲里指明的C++开发工具。
但是 Dev-C++ 已死于2016年，能安装的版本对 C++ 的支持过于古旧，因此不考虑。
...
@handbook-scenario{项目制课程相关}

本小节列出的软件仅参加项目制课程的学生需要安装，而且是必须安装，没有捷径。
