...
本引擎的外围工具提供了一个完整的测试框架，名为 specmon，
使用 Typed Racket 编写，
用于支持行为驱动开发(Behavior-Driven Development, BDD)工作流。
...
在本引擎中，适合自动化的最小可测单元主要包括数学对象和物理对象、
通信协议中的数据类型和校验码。
具体的开发工作流可参考@Secref{bdd:flow:ex}。
...
此外，本引擎自带一个类似课程项目软件的原型测试软件(名为 Tamer)，
...
所有实现了 @:id{IValuelet} 接口的可视元素都自带该能力。
