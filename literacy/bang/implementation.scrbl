...
本章的 @fig-ref{cosmos.dia} 不再关注 Matter 的子类，
而更关注可视元素如何被组织和管理。
...
@handbook-itemlist[
 ...
 @item{glide: 在指定时间内滑行到指定位置，使用相对位置参数。}
 @item{glide_to: 在指定时间内滑行到指定位置，使用绝对位置参数。}
 ]

@$$[#:tag "n(s)"]{ n = \lfloor s \times f \rfloor }

当前版本使用@eqref[#:label "公式"]{n(s)}将滑动时间 @${s} 映射为帧数 @${n}， @${f} 为帧频。
...
详见@tab-ref{anchor}和@fig-ref{gallery}。
...
在上课过程中，教师也可以顺便科普一下色度图和显示器失真问题。
本引擎提供两种配置的色度图对象，
它们的对比效果见@fig-ref{chromaticity}。

@tamer-figure!['chromaticity "色度图演示程序"]{
 @(let ([s 0.16])
    (list @(para @(stone-image "Bang/Chromaticity1.png" #:scale s) @elem{标准色度图})
          @(para @(stone-image "Bang/Chromaticity2.png" #:scale s) @elem{D65 色度图})))}
