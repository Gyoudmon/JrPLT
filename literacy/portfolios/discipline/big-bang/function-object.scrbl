...
@itemlist[
 ...
 @item{@handbook-defterm[#:origin "Function"]{函数}表示计算机要做的事。

  @tech{函数}可以类比成语言界“动宾结构”。
  在此视角下，@tech{函数}有“名称”和“参数列表”两个要素。
  “函数名”对应着“动作名称”；
  “参数列表”对应着“宾语(含补语)”，宾语的数量可能不唯一。

  @tech{函数}的第三个要素是“返回值”(@tech{函数}的执行结果)。
  有返回值的@tech{函数}的最典型例子是@handbook-defterm{数学函数}：
  根据一定的运算规则将输入参数转化为一个返回值。}

 @item{@handbook-defterm[#:origin "Variable"]{变量}即可以被改变的量，
  在计算机中它指的是内存地址，用来提示“这段内存中的内容可以修改”。
  可类比储物柜的格子(所在的@tech{位置})，
  但需要注意的是，@tech{变量}即是储物柜格子(所在的@tech{位置})本身，
  “变量名”才是贴在格子上的“标签”。}
...
@itemlist[
 ...
 @item{@tech{变量}(和@tech{常量})有两个属性：@tech{类型}和@tech{值}。
  @handbook-defterm[#:origin "Value"]{值}指的是某段内存里面的内容；
  @handbook-defterm[#:origin "Type"]{类型}则用于提示“如何解读这段内存里面的内容”。}
...
@itemlist[
 ...
 @item{@tech{类}可以比作设计图，声明为该@tech{类}的@tech{变量}即是根据此设计图制造出的产品，
