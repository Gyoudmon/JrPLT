...
@margin-note{Racket 的谓词函数通常以 @sign{?} 结尾。}
...
相对于草稿纸上的函数 @id{rpul}，
@id{find-top-calorie} 这个用 @form{define} 和 @id{λ} 定
义的正式函数除了包含@racket[<读取-判断-更新循环>]本体外，
...
@tamer-repl[#:requires ["../aoc.rkt"]
            ($ find-top-calorie #:< "mee/01_cc_ex.aoc" #:expect 24000)]
...
这个复数形式的 @id{find-top-calories} 函数增加了一个类型为 @type{Natural} 的参数 @variable{n}：
...
@tamer-repl[($ find-top-calories 3 #:< "mee/01_cc.aoc")]
