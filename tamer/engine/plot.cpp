...
/*************************************************************************************************/
void WarGrey::STEM::PlotPlane::load(float width, float height) {
...
void WarGrey::STEM::PlotPlane::reflow(float width, float height) {
    TheBigBang::reflow(width, height);
    
    this->move_to(this->history, width * 0.5F, height * 0.5F, MatterAnchor::CC);
}

void WarGrey::STEM::PlotPlane::update(uint64_t count, uint32_t interval, uint64_t uptime) {
    this->history_day ++;
