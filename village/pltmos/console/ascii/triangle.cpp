...
/*************************************************************************************************/
WarGrey::PLT::CharIsoscelesTriangle::CharIsoscelesTriangle(int term_row, int term_col)
    : ASCIIArt(term_row - 1, term_col) {}

int WarGrey::PLT::CharIsoscelesTriangle::pos(int l) const noexcept {
    return this->row - l + 1;
}

int WarGrey::PLT::CharIsoscelesTriangle::span(int l) const noexcept {
    return 2 * l - 1;
}

/*************************************************************************************************/
std::string WarGrey::PLT::CharIsoscelesTriangle::model_desc() const noexcept {
    return "pos(L) = N - L + 1;\nspan(L) = 2L - 1;";
}

std::string WarGrey::PLT::CharIsoscelesTriangle::line_desc(int l) const noexcept {
    return make_nstring("  pos(%d) = %d - %d + 1 = %d;\nspan(%d) = 2 * %d - 1 = %d;",
                        l, this->row, l, this->pos(l), l, l, this->span(l));
}

std::string WarGrey::PLT::CharIsoscelesTriangle::shape_line(int l) const noexcept {
